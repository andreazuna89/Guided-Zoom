## SCRIPT TO COMPUTE AND SAVE THE SOFTMAX PREDICTED BY THE CONVENTIONAL AND THE EVIDENCE CNNs. THE EVIDENCE ARE EXTRACTED USING RISE SALIENCY
## COMMAND TO USE:
#CUDA_VISIBLE_DEVICES=0 python rise_save_softmax.py --CONV PATH_TO_CONV_MODEL --EVID PATH_TO_EVID_MODEL --arch resnet101 --n-classes DATA_CLASSES --data PATH_TO_DATA
...
parser = argparse.ArgumentParser(description='PyTorch GUIDED ZOOM inference using RISE saliency')
...
def main():
    ...
    explainer = RISE(net_conventional, (image_dim_conventional, image_dim_conventional), 128)
    if not os.path.isfile('masks.npy'):
        explainer.generate_masks(1000, 8, 0.5, 'masks.npy')
    else:
    ...
    output = open( 'softmax_saved/'+args.data.split('/')[-2] +'_softmax_RISE.pkl', 'wb')
    ...
    with torch.no_grad():
      for i, (batch, label) in enumerate(tqdm(loader, total=len(loader))):
        ...
        for j in range(out.shape[0]):
          sals.append(explainer(batch[j].unsqueeze(0).cuda()))
        for cntr in range(top_classes):
          attMaps_single_batch=np.zeros((out.shape[0],image_dim_conventional,image_dim_conventional),dtype=float)
          for j in range(out.shape[0]):
        ...
        for j in range(out.shape[0]): 
          ...
          for jj in range(top_classes):
            [x,y] = np.unravel_index(attMaps[jj][j,:].argmax(), attMaps[jj][j,:].shape)
        ...
        for cntr in range(top_classes):
          ...
          for j in range(out.shape[0]): 
            ...
            [x,y] = np.unravel_index(attMaps[cntr][j].argmax(), attMaps[cntr][j].shape)
            ...
            sal = explainer(img.cuda())[tagID_top[cntr,j]].cpu().numpy()
            attMaps_single_batch[j,:]=sal[0]
        ...
        for j in range(out.shape[0]): 
          patch_data = torch.zeros(top_classes,3,image_dim_evidence,image_dim_evidence)
          for jj in range(top_classes):
            [x,y] = np.unravel_index(attMaps2[jj][j,:].argmax(), attMaps2[jj][j,:].shape)
            begin_x = max(0, x - patch_size / 2)
            end_x = min(image_dim_conventional - 1, x + patch_size / 2)
            begin_y = max(0, y - patch_size / 2)
            end_y = min(image_dim_conventional - 1, y + patch_size / 2)
        ...
        for cntr in range(top_classes):
          ...
          for j in range(out.shape[0]): 
            original_data_evidence=batch_orig[j].copy()
            [x1,y1] = np.unravel_index(attMaps[cntr][j].argmax(), attMaps[cntr][j].shape)
            [x2,y2] = np.unravel_index(attMaps2[cntr][j].argmax(), attMaps2[cntr][j].shape)
        ...
        for j in range(out.shape[0]): 
          patch_data = torch.zeros(top_classes,3,image_dim_evidence,image_dim_evidence)
          for jj in range(top_classes):
            [x,y] = np.unravel_index(attMaps3[jj][j,:].argmax(), attMaps3[jj][j,:].shape)
            begin_x = max(0, x - patch_size / 2)
            end_x = min(image_dim_conventional - 1, x + patch_size / 2)
            begin_y = max(0, y - patch_size / 2)
            end_y = min(image_dim_conventional - 1, y + patch_size / 2)
