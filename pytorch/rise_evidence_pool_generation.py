## SCRIPT FOR EVIDENCE POOL GENERATION. THE EVIDENCE ARE EXTRACTED USING RISE SALIENCY
## COMMAND TO USE:
#CUDA_VISIBLE_DEVICES=0 python rise_evidence_pool_generation.py --model PATH_TO_CONV_MODEL --arch resnet101 --n-classes DATA_CLASSES --data PATH_TO_DATA
...
parser = argparse.ArgumentParser(description='PyTorch GUIDED ZOOM patches extraction using RISE saliency')
...
def main():
    ...
    explainer = RISE(model, (448, 448), 128)
    ...
    for folder in os.listdir(dir):
      if not os.path.isdir(os.path.join('RISE_saliency',args.data.split('/')[-2], folder)):
            os.makedirs(os.path.join('RISE_saliency',args.data.split('/')[-2], folder))
      if not os.path.isdir(os.path.join('RISE_patches',args.data.split('/')[-2], folder)):
            os.makedirs(os.path.join('RISE_patches',args.data.split('/')[-2], folder))
    with torch.no_grad():
      for i, (img, label) in enumerate(tqdm(loader, total=len(loader))):
        ...
        while((torch.argmax(out).cpu()==label or counter==0) and counter<3):
          counter+=1
          sal = explainer(img.cuda())[label].cpu().numpy()
          
          sal = Image.fromarray(sal[0])
          ...
          plt.imsave(os.path.join('RISE_patches', args.data.split('/')[-2],folder, str(counter)+'_'+file), img_orig[begin_x:end_x,begin_y:end_y])
        ...
        #---------------------------------------------------------------- SALIENCY
          ...
          plt.imsave(os.path.join('RISE_saliency',args.data.split('/')[-2],folder,str(counter)+'_'+file), attMap)
        #----------------------------------------------------------------
